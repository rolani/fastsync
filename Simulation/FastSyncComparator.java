...
public class FastSyncComparator extends Items {
...
//	static String OUTFILE1 = "output.txt"; // inter-sync time
	...
	public static void fast_scheduler_() {
		...
		while (z < SIMULATION_ROUNDS) {
			...
			if (z % 5 == 0) {
				...
				CLUSTER_COST = getTaskGaussian(CLUSTER_RECONFIGURATION, 2);
				COMMUNICATION_COST += (getTaskGaussian(WORKER_CONTROLLER_COMMUNICATION, 2) * 2) +
						((INIT_DEVICES -1) * LOCAL_COMMUNICATION);
				COMMUNICATION_COST += getTaskGaussian(LOCAL_COMMUNICATION, 2);
			...
			log("Runtime per sync point is:" + runtime + " Comm cost is: " + COMMUNICATION_COST + 
					"Cluster cost: " + CLUSTER_COST);
			writer(df.format(runtime) + " " + df.format(COMMUNICATION_COST) +
					" " + df.format(CLUSTER_COST), OUTFILE1);
	...
	public static void getSyncOptions(Task t) {
		...
		if (quorumMet(ts1)) {
			...
			COMMUNICATION_COST += 3 * getTaskGaussian(LOCAL_COMMUNICATION, 0.2);
