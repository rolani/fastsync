...
public class SSPSyncExecutor extends Items {
...
//	static double STALE_VALUE = 3;
	
//	static String OUTFILE2 = "succ.txt"; // success/failure
//	static String OUTFILE3 = "output.txt";
	...
	// loop through tasks
	public static void ssp_scheduler_() {
		...
		while (z < SIMULATION_ROUNDS) {
			...
			for (Task t: taskQueue) {
				...
				if (t.type == TAG_C_ASYNC) {
					...
					checkStaleness();
					if (staleness_check == true)
						readyMachines.forEach(d -> {						
							COMMUNICATION_COST = 2*getTaskGaussian(WORKER_CONTROLLER_COMMUNICATION, 2);
						});
					scheduleSyncTask(t);
					sync_task_counter++;					
	...
	public static void scheduleSyncTask(Task t) {
		...
		for (Worker worker: readyMachines) {
			...
			worker.increaseTime(2); //communication cost
			runTask(worker, t, (time+(worker.sync_deviation)/4), 1); 
		}
		
	}	
	
	public static void checkStaleness() {
		//double fastest_ = getMin(getCurrentTimes(readyMachines));
		double slowest_ = getMax(getCurrentTimes(readyMachines));
		
		readyMachines.forEach(d ->{
			if (slowest_ - d.getTime() > STALE_VALUE * 5) {
				//d.increaseTime(WORKER_CONTROLLER_COMMUNICATION);
				staleness_check = true;
				d.setTime(slowest_ - STALE_VALUE);
	...
	public static void main(String[] args) {
		...
		try {
			...
			OUTFILE1 = args[2];
			STALE_VALUE = Double.parseDouble(args[3]); 
			SMALL_VARIANCE = Double.parseDouble(args[4]);
			LOCAL_COMMUNICATION = Double.parseDouble(args[5]);
			WORKER_CONTROLLER_COMMUNICATION = Double.parseDouble(args[6]);
